...
/**
 *  1. 根据检测级别和采样率的设置，在需要时检测 ByteBuf 创建的 WeakRefrence 引用
 *  2. 当 JVM 回收 ByteBuf 对象时，JVM 会将 WeakRefrence 放入 RefrenceQueue 队列中
 *  3. 当通过 ReferenceQueue 中 WeakRefrence 的检查，判断在 GC 前是否有释放 ByteBuf 的资源，就可以知道是否有资源释放
 * @param <T>
 */
public class ResourceLeakDetector<T> {
    ...
    /**
     * 每个 DefauleResourceLeak 记录的 Record 数量 默认 4
     */
    ...
    /**
     * 给指定资源（例如 ByteBuf 对象)创建一个检测他是否泄露的 ResourceLeakTracker 对象
     * @param obj
     * @return
     */
    @SuppressWarnings("unchecked")
